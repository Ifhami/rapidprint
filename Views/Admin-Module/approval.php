...
// Fetch students with pending verification from the 'registration' table
$sql = "SELECT r.UserID, r.full_name, r.verification_status, c.verification_proof 
        FROM registration r 
        LEFT JOIN customer c ON r.UserID = c.UserID 
        WHERE r.role = 'student' AND r.verification_status = 'pending' 
        LIMIT ? OFFSET ?";
...
$total_count_sql = "SELECT COUNT(*) AS total FROM registration WHERE role = 'student' AND verification_status = 'pending'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['selected_students']) && !empty($_POST['selected_students'])) {
    ...
    // Prepare SQL query for bulk update
    $placeholders = implode(',', array_fill(0, count($student_ids), '?'));
    $status = ($action === 'accept') ? 'completed' : 'rejected';
    $sql = ($action === 'reject') ? 
        "UPDATE registration SET verification_status = ?, verification_proof = NULL WHERE UserID IN ($placeholders)" :
        "UPDATE registration SET verification_status = ? WHERE UserID IN ($placeholders)";
    
    // Execute SQL with transaction
    $conn->begin_transaction();
    $stmt = $conn->prepare($sql);
    $types = str_repeat("i", count($student_ids));
    $stmt->bind_param("s" . $types, $status, ...$student_ids);
